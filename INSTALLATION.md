...
Extract the ZIP before opening `Cleo.exe`; do not run it from inside the ZIP. Windows 10 and
Windows 11 both include ZIP extraction in File Explorer, so no third-party unzip program is
required. Right-click the downloaded ZIP, select **Extract All**, and follow the prompt.
